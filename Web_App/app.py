...
@app.callback(
    Output("state-btn", "children"),
    Output("state-btn", "color"),
    Input("state-btn", "n_clicks"),
    prevent_initial_call=True
)
def toggle_state(n_clicks):
    if n_clicks % 2 == 1:
        return "STATE: ON", "success"
    return "STATE: OFF", "secondary"
...
@app.callback(
    ...
    State("state-btn", "children"),
    prevent_initial_call=True
)
def send_control(n, setpoint, mode, state):
    ...
    return f"(Sent) Mode: {payload['mode']} State: {payload['state']} Huminity Threshold {payload['humi_thres']}" \
