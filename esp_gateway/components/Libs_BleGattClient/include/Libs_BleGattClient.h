...
typedef struct {
    ...
    // char handles
    uint16_t h_temp_val, h_humi_val, h_co2_val, h_state_val, h_mode_val;
    uint16_t h_temp_cccd, h_humi_cccd, h_co2_cccd, h_state_cccd, h_mode_cccd;
    uint16_t h_led_val, h_led_mode_val;
} peer_t;
...
void gateway_set_state(int server_idx, uint8_t state);
